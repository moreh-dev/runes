...
- **Commit:** one commit per applied thread, made immediately after the change. If multiple threads are duplicates or are resolved by the same cohesive edit, one commit may cover them together; reply with the same SHA only when that commit literally contains each fix. Conventional Commits, single scope. Pick the type by change kind — `fix` (bug), `refactor` (cleanup), `docs` (docs-only), `test` (test changes), `style` (formatting-only). Scope is the top-level directory of the primary modified file (e.g., `skills`, `cmd`, `api`).
